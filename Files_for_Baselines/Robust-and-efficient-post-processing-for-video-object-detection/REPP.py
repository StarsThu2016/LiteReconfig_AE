...
class REPP():
    ...
    def __call__(self, video_predictions):
        ...
        if self.store_imdb: predictions_ours = self.tubelets_to_predictions(tubelets, 'ours')
        else: predictions_ours = []
        
        return predictions_coco, predictions_imdb, predictions_ours
...
if __name__ == '__main__':
    ...
    parser.add_argument('--store_ours', help='store processed predictions in imdb format', action='store_true')
    ...
    total_preds_coco, total_preds_imdb, total_preds_ours = [], [], []
    print(' * Applying repp')
    if args.evaluate:
    ...
    for vid, video_preds in get_video_frame_iterator(args.predictions_file, from_python_2=args.from_python_2):
        predictions_coco, predictions_imdb, predictions_ours = repp(video_preds)
        total_preds_coco += predictions_coco
        total_preds_imdb += predictions_imdb
        total_preds_ours += predictions_ours
    ...
    if args.store_ours:
        print(' * Dumping predictions with the COCO format:', predictions_file_out + '_ours.json')
        json.dump(total_preds_coco, open(predictions_file_out + '_ours.json', 'w'))
