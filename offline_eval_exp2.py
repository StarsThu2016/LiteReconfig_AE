...
def get_latency_from_ApproxDet(filename):
    
    # Extract the mean and std of latency in GoF granularity, ignore incomplete GoFs
    column_dict = {"is_det_frame": 3, "si": 6, "lat_detection": 13, "lat_tracker": 14, "lat_overhead": 12}
...
filename = "offline_logs_AE/executor_ApproxDet_g0_lat100_tx2_lat.txt"
_, _, lat_p95 = get_latency_from_ApproxDet(filename)
filename = "offline_logs_AE/executor_ApproxDet_g0_lat100_tx2_det.txt"
meanAP = compute_mAP(gt="test/VID_testgt_full.txt", detection=filename)
print(f"ApproxDet, no contention: {meanAP*100:.1f}% mAP, {lat_p95:.1f} ms p95 latency.")

filename = "offline_logs_AE/executor_ApproxDet_g50_lat100_tx2_lat.txt"
_, _, lat_p95 = get_latency_from_ApproxDet(filename)
filename = "offline_logs_AE/executor_ApproxDet_g50_lat100_tx2_det.txt"
meanAP = compute_mAP(gt="test/VID_testgt_full.txt", detection=filename)
print(f"ApproxDet, 50% GPU contention: {meanAP*100:.1f}% mAP, {lat_p95:.1f} ms p95 latency.")
...
filename = "offline_logs_AE/executor_LiteReconfig_g0_lat100_tx2_lat.txt"
_, _, lat_p95 = get_latency_from_LiteReconfig(filename)
filename = "offline_logs_AE/executor_LiteReconfig_g0_lat100_tx2_det.txt"
meanAP = compute_mAP(gt="test/VID_testgt_full.txt", detection=filename)
print(f"LiteReconfig, no contention: {meanAP*100:.1f}% mAP, {lat_p95:.1f} ms p95 latency.")

filename = "offline_logs_AE/executor_LiteReconfig_g50_lat100_tx2_lat.txt"
_, _, lat_p95 = get_latency_from_LiteReconfig(filename)
filename = "offline_logs_AE/executor_LiteReconfig_g50_lat100_tx2_det.txt"
meanAP = compute_mAP(gt="test/VID_testgt_full.txt", detection=filename)
