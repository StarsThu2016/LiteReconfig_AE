...
class VIDDemo(object):
    ...
    def run_on_image_folder(self, image_folder, suffix='.JPEG'):
        ...
        detoutput_filename = "test_MEGA_det.txt"
        latoutput_filename = "test_MEGA_lat.txt"
        ...
        # # Preprocess the list of images to mimick the behaviour
        # for img_path in imgs_list:
        # #     vid_name = img_path.split('/')[-2]
        # #     if old_vid_name != vid_name:
        # #         img_folder_list.append(temp)
        # #         temp = []
        ...
        for img_path in tqdm(imgs_list):
            ...
            if self.method == "base":
                ...
                time1 = time.time()
                results = self.run_on_image(original_image, img_cur)
                time2 = time.time()

                inf_lat = (time2-time1)*1e3
                # # print(float(results.bbox[0][0])) # xmin, ymin, xmax, ymax
                # print(results.bbox)
                # print(results.get_field('labels'))
                # print(results.num_boxes)
                for i in range(len(results.bbox)):
                    xmin = float(results.bbox[i][0])
                    ymin = float(results.bbox[i][1])
                    xmax = float(results.bbox[i][2])
                    ymax = float(results.bbox[i][3])
                    # print(int(results.get_field('labels')))
                    label = int(results.get_field('labels')[i]) - 1
                    # print(results.get_field('bbox'))
                    # print(results.get_field('scores'))
                    conf = float(results.get_field('scores')[i])
                    print("{} {} {} {} {} {} {}".format(img_path, label, conf, ymin/h, xmin/w, ymax/h, xmax/w), file=fout_det)
                    print(inf_lat, file=fout_lat)
                ...
                time1 = time.time()
                results = self.run_on_image(original_image, infos)
                time2 = time.time()
                ...
                # print("{} {} {} {} {} {} {} {}".format(img_path, label, conf, ymin/h, xmin/w, ymax/h, xmax/w, inf_lat), file=fout_det)
                # images_with_boxes.append(image_with_boxes)
            else:
